...
const CreateTourView: React.FC = () => {
    ...
    const { popularity, songs, hype } = activeArtistData;
    ...
    const handleCreateTour = () => {
        ...
        // Estimate demand
        const totalCap = finalVenues.reduce((sum, v) => sum + v.capacity, 0);
        const baseDemand = (popularity * 800) + (hype * 50); 
        const priceSensitivity = 1.2 - (ticketPrice / 200);
        let demand = baseDemand * Math.max(0.1, priceSensitivity);
        
        // Convert to a 0-100 score for UI
        const demandScore = Math.min(100, Math.round((demand / (totalCap || 1)) * 100));
        ...
        const newTour: Tour = {
            ...
            status: 'presale', // Starts in presale
            currentVenueIndex: 0,
            totalRevenue: 0,
            ticketsSold: 0,
            useDynamicPricing,
            ...
            case 3: // Setlist
                ...
                return (
                     <div className="space-y-4">
                        ...
                        <button onClick={() => setStep(4)} className="w-full bg-red-600 hover:bg-red-500 transition-colors p-3 rounded-lg font-bold">Next: Presale</button>
                    </div>
                );
            case 4: // Presale
                return (
                    <div className="space-y-4">
                        ...
                        <button onClick={handleCreateTour} className="w-full bg-blue-600 hover:bg-blue-500 p-3 rounded-lg font-bold transition-colors">Setup Tour</button>
                    </div>
                )
        }
    };
    ...
    return (
        <div className="h-screen w-full bg-zinc-900 overflow-y-auto text-white">
            <header className="p-4 flex items-center gap-4 sticky top-0 bg-zinc-900/80 backdrop-blur-sm z-10 border-b border-zinc-700/50">
                <button onClick={() => step > 1 ? setStep(s => s - 1) : dispatch({type: 'CHANGE_VIEW', payload: 'tours'})} className="p-2 rounded-full hover:bg-white/10 transition-colors">
                    <ArrowLeftIcon className="w-6 h-6" />
                </button>
                <h1 className="text-2xl font-bold">Create Tour</h1>
            </header>
            <main className="p-4">
                {error && <p className="text-red-400 mb-4 bg-red-900/20 p-3 rounded-lg border border-red-500/50">{error}</p>}
                {renderStep()}
            </main>
        </div>
    );
