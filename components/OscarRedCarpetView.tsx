...
const OscarRedCarpetView: React.FC = () => {
    const { gameState, dispatch } = useGame();
    const { activeOscarRedCarpetOffer } = gameState;

    const [lookUrl, setLookUrl] = useState<string | null>(null);

    if (!activeOscarRedCarpetOffer) return <p>Error</p>;
    ...
    const handleAttend = () => {
        if (lookUrl) {
            dispatch({ type: 'ACCEPT_OSCAR_RED_CARPET', payload: { emailId: activeOscarRedCarpetOffer.emailId, lookUrl }});
        }
    };
    ...
    return (
        <div className="h-full w-full bg-zinc-900 flex flex-col">
             <header className="p-4 flex items-center gap-4 sticky top-0 bg-zinc-900/80 backdrop-blur-sm z-10 border-b border-zinc-700/50">
                <button onClick={() => dispatch({type: 'DECLINE_OSCAR_RED_CARPET', payload: {emailId: activeOscarRedCarpetOffer.emailId}})} className="p-2 rounded-full hover:bg-white/10">
                    <ArrowLeftIcon className="w-6 h-6" />
                </button>
                <h1 className="text-xl font-bold">Oscars Red Carpet</h1>
            </header>
...
export default OscarRedCarpetView;
