...
const ChartRow: React.FC<{ entry: ChartEntry }> = ({ entry }) => {
    ...
    return (
        <div className="grid grid-cols-[auto_auto_1fr_auto] sm:grid-cols-[auto_auto_1fr_repeat(4,_minmax(0,_1fr))] items-center gap-2 sm:gap-4 py-2 text-sm">
            ...
            <div className="text-right text-zinc-400">{formatNumber(weeklyStreams)}</div>
        </div>
    );
};
...
const SpotifyTopSongsView: React.FC = () => {
    ...
    const { spotifyGlobal50, date } = gameState;
        ...
        <div className="bg-[#121212] min-h-screen text-white">
            ...
            {highestNewEntry && (
                <div className="p-4 mt-2">
                    <div className="bg-rose-800 rounded-lg p-4 flex justify-between items-center">
                        <div className="w-2/3">
                            <p className="text-lg font-bold">“{highestNewEntry.title}” by {highestNewEntry.artist} is the highest new entry on Top Songs Global at #{highestNewEntry.rank}.</p>
                            <p className="text-xs opacity-80 mt-1">Top Songs Global · Week of {date.week}</p>
                        </div>
            ...
            <main className="p-4">
                <div className="flex justify-between items-center">
                    <h2 className="text-xl font-bold">Weekly Top Songs Global</h2>
                ...
                <p className="text-sm text-zinc-400">Your weekly update of the most played tracks right now.</p>
                
                <div className="flex gap-2 mt-4">
                    <button className="bg-zinc-700 px-3 py-1 rounded-full text-sm font-semibold flex items-center gap-1">Weekly <ChevronDownIcon className="w-4 h-4"/></button>
                ...
                <div className="mt-4 border-b border-zinc-700 pb-2 text-xs text-zinc-500 font-semibold grid grid-cols-[auto_auto_1fr_auto] sm:grid-cols-[auto_auto_1fr_repeat(4,_minmax(0,_1fr))] gap-2 sm:gap-4">
                ...
                <div className="divide-y divide-zinc-800">
                    {spotifyGlobal50.map(entry => <ChartRow key={entry.uniqueId} entry={entry} />)}
                </div>
            </main>
        </div>
    );
