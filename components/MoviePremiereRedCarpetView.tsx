...
const MoviePremiereRedCarpetView: React.FC = () => {
    ...
    const handleAttend = () => {
        if (lookUrl) {
            dispatch({ type: 'ACCEPT_MOVIE_PREMIERE_RED_CARPET', payload: { emailId: activeMoviePremiereOffer.emailId, lookUrl }});
        }
    };
    ...
    return (
        <div className="h-full w-full bg-zinc-900 flex flex-col">
             <header className="p-4 flex items-center gap-4 sticky top-0 bg-zinc-900/80 backdrop-blur-sm z-10 border-b border-zinc-700/50">
                <button onClick={() => dispatch({type: 'DECLINE_OSCAR_RED_CARPET', payload: {emailId: activeMoviePremiereOffer.emailId}})} className="p-2 rounded-full hover:bg-white/10">
