...
const CreateGrammyPerformanceView: React.FC = () => {
    const { gameState, dispatch, activeArtist, activeArtistData } = useGame();
    const { date, activeGrammyPerformanceOffer } = gameState;
    ...
    if (!activeArtist || !activeGrammyPerformanceOffer) {
    ...
    const handlePublish = () => {
        ...
        const videoTitle = `${activeArtist.name} Performs "${songTitles}" at the GRAMMYs`;
        
        const description = `Watch ${activeArtist.name} perform a medley of "${songTitles}" live from the ${date.year + 1} GRAMMY Awards.`;
        ...
        const newVideo: Video = {
            ...
            views: 0,
            thumbnail,
            releaseDate: { week: 52, year: date.year },
            artistId: activeArtist.id,
            description,
            mentionedNpcs: [],
        };

        dispatch({ type: 'CREATE_GRAMMY_PERFORMANCE', payload: { video: newVideo } });
    };

    return (
        <div className="h-full w-full bg-zinc-900 flex flex-col">
            <header className="p-4 flex items-center gap-4 sticky top-0 bg-zinc-900/80 backdrop-blur-sm z-10 border-b border-zinc-700/50">
                <button onClick={() => dispatch({ type: 'DECLINE_GRAMMY_PERFORMANCE', payload: { emailId: activeGrammyPerformanceOffer.emailId }})} className="p-2 rounded-full hover:bg-white/10">
                    <ArrowLeftIcon className="w-6 h-6" />
                </button>
                <h1 className="text-xl font-bold">Plan GRAMMY Performance</h1>
            </header>
...
export default CreateGrammyPerformanceView;