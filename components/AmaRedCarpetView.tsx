...
const AmaRedCarpetView: React.FC = () => {
    const { gameState, dispatch } = useGame();
    const { activeAmaRedCarpetOffer } = gameState;

    const [lookUrl, setLookUrl] = useState<string | null>(null);

    if (!activeAmaRedCarpetOffer) return <p>Error</p>;
    ...
    const handleAttend = () => {
        if (lookUrl) {
            dispatch({ type: 'ACCEPT_AMA_RED_CARPET', payload: { emailId: activeAmaRedCarpetOffer.emailId, lookUrl }});
        }
    };
...
export default AmaRedCarpetView;
