...
state_inject = """    const [isCreatingCommunity, setIsCreatingCommunity] = useState(false);
    const [communityName, setCommunityName] = useState('');
    const [selectedPost, setSelectedPost] = useState<InstagramPost | null>(null);
    const [selectedReel, setSelectedReel] = useState<InstagramReel | null>(null);"""

content = content.replace("    const [isCreatingCommunity, setIsCreatingCommunity] = useState(false);\n    const [communityName, setCommunityName] = useState('');", state_inject)

render_inject = """    const myCommunities = activeArtistData.communities || [];
...
content = content.replace("    const myCommunities = activeArtistData.communities || [];", render_inject)

old_post_click = "onClick={() => dispatch({ type: 'DELETE_INSTAGRAM_POST', payload: { postId: post.id } })}"
new_post_click = "onClick={() => setSelectedPost(post)}"
content = content.replace(old_post_click, new_post_click)

old_reel_click = "onClick={() => dispatch({ type: 'DELETE_INSTAGRAM_REEL', payload: { reelId: reel.id } })}"
new_reel_click = "onClick={() => setSelectedReel(reel)}"
content = content.replace(old_reel_click, new_reel_click)
